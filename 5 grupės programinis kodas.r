...
tikslumas<-function(modelis){
  ...
  ggplot(roc_df, aes(x = Specificity, y = Sensitivity)) +
    geom_step(color = "#1F77B4", linewidth = 1.8, direction = "hv") +  
...
data1 %>%
  ...
  labs( y = "Logoritmuota  reikšmė", x='',fill = "") +  
  theme_minimal() +  
...
isskirtys(logit_modelis) #isskirciu yra, jas saliname
...
isskirtys(logit_modelis1) #isskirciu nebera
...
summary(logit_modelis_num_words) #num_spelling_errors nereiksmingas
...
summary(logit_modelis_num_words) #num_words nereiksmingas
vif(logit_modelis_num_words)
...
summary(logit_modelis_num_stopwords) #num_stopwords nereiksmingas
...
summary(logit_modelis_num_stopwords) #num_spelling_errors nereiksmingas
vif(logit_modelis_num_stopwords)
...
summary(logit_modelis_num_links) #num_spelling_errors nereiksmingas
...
summary(logit_modelis_num_unique_domains) #num_spelling_errors nereiksmingas
...
tikslumas(logit_modelis_num_unique_words)
...
isskirtys(probit_modelis) #isskirciu yra
...
summary(probit_modelis_num_words) # num_spelling_errors nereiksmingas
...
summary(probit_modelis_num_words) # num_words nereiksmingas
vif(probit_modelis_num_words)
...
summary(probit_modelis_num_stopwords) #num_stopwords nereiksmingas
...
summary(probit_modelis_num_stopwords) #num_spelling_errors nereiksmingas
vif(probit_modelis_num_stopwords)
...
summary(probit_modelis_num_links) #num_spelling_errors nereiksmingas
...
tikslumas(probit_modelis_num_unique_domains)
...
################################################################################################
###### Modelis ivykiu skaiciui prognozuoti ######
################################################################################################
data1 %>%
  ...
  #scale_x_discrete(breaks = as.character(seq(0, 78000, by = 100))) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(hjust = 0.5))

data1<-data1%>%filter(num_spelling_errors<=50) #isrenkame tik stebejimus kur numnum_spelling_errors<=50
...
var(datatrain$num_spelling_errors) #nesutampa, greiciausiai puasono regresija netiks
...
isskirtys(neg_binom_model) #yra išskircių, jas pasaliname
...
model_3 <- glm.nb(num_spelling_errors ~ num_stopwords + num_links + num_email_addresses +
                  ...
                  data = datatrain2,
                  control = glm.control(maxit = 350))
summary(model_3) #num_urgent_keywords  nereiksminga
...
model_2_saveika <- glm.nb(num_spelling_errors ~ num_unique_words + num_links*num_email_addresses +
                  ...
                  data = datatrain2,
                  control = glm.control(maxit = 350))
summary(model_2_saveika) #saveika stat. reiksminga

AIC(model_2)
AIC(model_2_saveika)
anova(model_2, model_2_saveika) #Skirtumas statistiskai reiksmingas
